...
describe('BalanceService', () => {
  it('returns balance response with all dimensions', async () => {
    const service = new BalanceService(makeFactory(makeAdapter(168)) as any);
  ...
  it('calls getAdapter with the correct employerId', async () => {
    ...
    const service = new BalanceService(factory as any);
    await service.getBalance(
      PARAMS.employeeId, PARAMS.employerId, PARAMS.locationId, PARAMS.leaveType, PARAMS.year,
    );
  ...
  it('calls adapter.getBalance with the full BalanceKey', async () => {
    ...
    const service = new BalanceService(makeFactory(adapter) as any);
    await service.getBalance(
      PARAMS.employeeId, PARAMS.employerId, PARAMS.locationId, PARAMS.leaveType, PARAMS.year,
    );
  ...
  it('propagates HcmUnavailableException from adapter', async () => {
    const adapter = { getBalance: jest.fn().mockRejectedValue(new HcmUnavailableException()) };
    const service = new BalanceService(makeFactory(adapter) as any);
    await expect(
      service.getBalance(PARAMS.employeeId, PARAMS.employerId, PARAMS.locationId, PARAMS.leaveType, PARAMS.year),
    ).rejects.toBeInstanceOf(HcmUnavailableException);
  });

  it('propagates HcmDomainException from adapter', async () => {
    const adapter = { getBalance: jest.fn().mockRejectedValue(new HcmDomainException('invalid dims')) };
    const service = new BalanceService(makeFactory(adapter) as any);
    await expect(
      service.getBalance(PARAMS.employeeId, PARAMS.employerId, PARAMS.locationId, PARAMS.leaveType, PARAMS.year),
  ...
  it('propagates HcmConfigNotFoundException from factory', async () => {
    const factory = { getAdapter: jest.fn().mockRejectedValue(new HcmConfigNotFoundException('er-1')) };
    const service = new BalanceService(factory as any);
    await expect(
      service.getBalance(PARAMS.employeeId, PARAMS.employerId, PARAMS.locationId, PARAMS.leaveType, PARAMS.year),
