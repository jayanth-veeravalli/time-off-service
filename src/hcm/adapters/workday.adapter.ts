...
export class WorkdayAdapter implements IHcmAdapter {
  constructor(private readonly baseUrl: string) {}

  async getBalance(params: BalanceKey): Promise<number> {
    try {
      const response = await axios.get<{ balanceHours: number }>(`${this.baseUrl}/balance`, {
        params,
        timeout: TIMEOUT_MS,
      });
  ...
  async debitBalance(
    ...
    try {
      await axios.post(`${this.baseUrl}/debit`, params, { timeout: TIMEOUT_MS });
    } catch (err) {
      throw mapAxiosError(err);
    }
  }
  ...
  async reverseDebit(
    ...
    try {
      await axios.post(`${this.baseUrl}/reverse`, params, { timeout: TIMEOUT_MS });
    } catch (err) {
      throw mapAxiosError(err);
    }
  }
