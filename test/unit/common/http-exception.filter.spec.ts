...
describe('HttpExceptionFilter', () => {
  ...
  it('maps OverlapConflictException to 409 OVERLAP_CONFLICT', () => {
    ...
    expect(json).toHaveBeenCalledWith(expect.objectContaining({ code: 'OVERLAP_CONFLICT' }));
  });
  ...
  it('verifies 404 status for RequestNotFoundException', () => {
    ...
    const h = {
      switchToHttp: () => ({ getResponse: () => ({ status: statusFn, json: jest.fn() }) }),
    } as unknown as ArgumentsHost;
